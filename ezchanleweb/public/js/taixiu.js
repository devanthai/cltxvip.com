  ...
  $.ajax({
    ...
    success: function (res) {
      ...
      if (res.game) {
        ...
        if (gamez.server == 1) {
          server.innerHTML = "Trùm CLTX"
        }
        else if (gamez.server == 2) {
          server.innerHTML = "Liên Server"
        }
        else if (gamez.server == 3) {
          server.innerHTML = "Server 24/24"
        }
      ...
      if (res.user) {
        goldUser.value = numberWithCommas(res.user.vang)
        userInfo.innerHTML = '<i class="fas fa-user"></i> ' + res.user.name + " - " + numberWithCommas(res.user.vang) + "$"
      }
      if (res.cuoc) {
