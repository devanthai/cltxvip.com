    ...
    try {
        ...
        if (getClan) {
            ...
            if (getMember) {
                ...
                if (getMember[index].username == undefined) {
                ...
                const message = '<div class="alertInfoContent" style="padding-top: 5px;">' +
                    ...
                    '<hr class="mt-2 mb-2">' +
                    '<div>' +
                    '<p style="font-weight: bold; color: red; text-align: center; margin-bottom: 0px;">Bang chủ: ' + getMember[index].username +
                    '</p>' +
...
router.post('/join', async (req, res) => {
    ...
    if (user) {
        const checkTime = await Chatclan.findOne({ type: type, uidclan: clanid, name: user.username }).sort({ time: -1 })
        if (checkTime) {
        ...
        const newChat = new Chatclan({ noidung: req.user.name, type: type, admin: 0, name: user.username, sodu: user.vang, uidclan: clanid })
        ...
        if (checkUserClan) {
            ...
            checkUserClan.forEach(element => {
                ...
                if (element._id.toString() === checkclan.uid.toString()) {
                    clannz += '<div class="ptItem"><div class="row" style="margin: 0;"><div class="col-6" style="padding-left: 5px; padding-right: 0px;"><p class="ptScreenName" style="color: red">' + element.username + '</p><small class="ptScrenText">Thành tích: ' + formatNumber(element.clan.thanhtich)+ (element.clan.thanhtichngay!=undefined ? " - Ngày: "+formatNumber(element.clan.thanhtichngay):"") + '</small></div><div class="col-6 text-right" style="padding-right: 5px; padding-left: 0px"><small style="color: red">Số dư: ' + numberWithCommas(Math.round(element.vang)) + '</small><br><small class="ptScrenText">Tham gia: ' + element.clan.time + '</small></div></div></div>';
                }
                else {
                    clannz += '<div class="ptItem"><div class="row" style="margin: 0;"><div class="col-6" style="padding-left: 5px; padding-right: 0px;"><p class="ptScreenName" style="color: green">' + element.username + '</p><small class="ptScrenText">Thành tích: ' + formatNumber(element.clan.thanhtich)+ (element.clan.thanhtichngay!=undefined ? " - Ngày: "+formatNumber(element.clan.thanhtichngay):"") + '</small></div><div class="col-6 text-right" style="padding-right: 5px; padding-left: 0px"><small style="color: green">Số dư: ' + numberWithCommas(Math.round(element.vang)) + '</small><br><small class="ptScrenText">Tham gia: ' + element.clan.time + '</small></div></div></div>';
...
router.post('/taobang', async (req, res) => {
    ...
    if (user) {
        if (user.clan != 0) {
            ...
            const soduuu = await UserControl.sodu(user._id, "-" + numberWithCommas(goldCreate), "Tạo bang hội")
            if (upxuuu) {
...
router.post('/chatbang', async (req, res) => {
    ...
    if (checkclan) {
        ...
        const user = await User.findById(req.user._id)
        if (user) {
            const newChat = new Chatclan({ noidung: message, type: type, admin: admin, name: user.username, sodu: user.vang, uidclan: idClan })
            const chatnew = await newChat.save();
            if (chatnew) {
                ...
                checkUserClan.forEach(element => {
                    ...
                    if (admin == 0) {
                        ...
                        if (element._id.toString() === checkclan.uid.toString()) {
                            ...
                            clannz += '<div class="ptItem" onclick="kickMember(\'' + element.username + '\')"><div class="row" style="margin: 0;"><div class="col-6" style="padding-left: 5px; padding-right: 0px;"><p class="ptScreenName" style="color: green">' + element.username + '</p><small class="ptScrenText">Thành tích: ' + formatNumber(element.clan.thanhtich) + (element.clan.thanhtichngay!=undefined ? " - Ngày: "+formatNumber(element.clan.thanhtichngay):"")+ '</small></div><div class="col-6 text-right" style="padding-right: 5px; padding-left: 0px"><small style="color: green">Số dư: ' + numberWithCommas(Math.round(element.vang)) + '</small><br><small class="ptScrenText">Tham gia: ' + element.clan.time + '</small></div></div></div>';
...
router.post('/chapnhan', async (req, res) => {
    ...
    if (findMess) {
        const checkUserrr = await User.findOne({ username: findMess.name })
        if (req.user.clan == 0) {
            return res.send({ status: 0, message: "Ko có bang mà đòi gì" });
        }
        ...
        const event = new Date().toLocaleDateString()
        const newDate = event.replace(/(\d+[/])(\d+[/])/, '$2$1');
        const USup = await User.findOneAndUpdate({ username: findMess.name }, { clan: { id: findMess.uidclan.toString(), time: newDate, thanhtich: 0 } })
        if (USup) {
            ...
            if (user) {
                ...
                const newChat = new Chatclan({ noidung: "Chấp nhận " + findMess.name + " vào bang", type: 3, admin: 1, name: user.username, sodu: user.vang, uidclan: user.clan.id })
                const chatnew = await newChat.save();
    ...
    try {
        ...
        if (findClan) {
            if (req.user._id.toString() != findClan.uid.toString()) {
                ...
                try {
                    const userChar = await User.findOne({ username: name })
                    if (userChar) {
                ...
                const kick = await User.findOneAndUpdate({ username: name }, { clan: 0 })


                const user = await User.findById(req.user._id)
                if (kick && user) {
                    const newChat = new Chatclan({ noidung: "Đã kick " + name + " ra khỏi bang hội", type: 2, admin: 1, name: user.username, sodu: user.vang, uidclan: user.clan.id })
                    const chatnew = await newChat.save();
                    if (chatnew) {
                        const id = req.user.clan.id
                        const checkclan = await Clan.findById(id)
                            ...
                            if (checkUserClan) {
                                ...
                                checkUserClan.forEach(element => {
                                    ...
                                    if (admin == 0) {
                                        ...
                                        if (element._id.toString() === checkclan.uid.toString()) {
                                            clannz += '<div class="ptItem"><div class="row" style="margin: 0;"><div class="col-6" style="padding-left: 5px; padding-right: 0px;"><p class="ptScreenName" style="color: red">' + element.username + '</p><small class="ptScrenText">Thành tích: ' + formatNumber(element.clan.thanhtich) + '</small></div><div class="col-6 text-right" style="padding-right: 5px; padding-left: 0px"><small style="color: red">Số dư: ' + numberWithCommas(element.vang) + '</small><br><small class="ptScrenText">Tham gia: ' + element.clan.time + '</small></div></div></div>';
                                        }
                                        else {
                                            clannz += '<div class="ptItem" onclick="kickMember(\'' + element.username + '\')"><div class="row" style="margin: 0;"><div class="col-6" style="padding-left: 5px; padding-right: 0px;"><p class="ptScreenName" style="color: green">' + element.username + '</p><small class="ptScrenText">Thành tích: ' + formatNumber(element.clan.thanhtich) + (element.clan.thanhtichngay!=undefined ? " - Ngày: "+formatNumber(element.clan.thanhtichngay):"")+ '</small></div><div class="col-6 text-right" style="padding-right: 5px; padding-left: 0px"><small style="color: green">Số dư: ' + numberWithCommas(element.vang) + '</small><br><small class="ptScrenText">Tham gia: ' + element.clan.time + '</small></div></div></div>';
...
router.post('/outpt', async (req, res) => {
    ...
    if (findClan) {
        if (req.user._id.toString() == findClan.uid.toString()) {
            ...
            const user = await User.findById(req.user._id)
            const newChat = new Chatclan({ noidung: req.user.name + " đã rời bang", type: 4, admin: 0, name: user.username, sodu: user.vang, uidclan: user.clan.id })
