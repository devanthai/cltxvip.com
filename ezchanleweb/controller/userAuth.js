...
router.post('/register', async (req, res) => {
    ...
    if (isNaN(avatar)) {
        return res.status(200).send({ error: 1, msg: 'Avatar không hợp lệ' })
    }
    else if (avatar < 1 || avatar > 10) {
        return res.status(200).send({ error: 1, msg: 'Máy chủ không hợp lệ' })
    }
    ...
    const user = new User({ username: username, password: hashPassword, server: server, avatar: Number(avatar) })
    try {
