...
router.post('/napvang', async (req, res) => {
    ...
    if (!req.user.isLogin) {
        ...
        if (type == 1) {
            ...
            if (getNapvang)
                return res.send('<tr><td hidden="">' + getNapvang._id + '</td><td>' + getNapvang.server + '</td><td>' + getNapvang.tnv + '</td><td>' + numberWithCommas(getNapvang.sothoi) + '</td><td>' + getStatus(getNapvang.status) + '</td><td>' + new Date(Date.parse(getNapvang.time)).toLocaleString() + '</td><td></td></tr>')
        ...
        if (type == 1) {
            ...
            const getBotnapvang = await Botnap.find({ server: req.user.server });
            ...
            const getBotnapvang = await Botthoi.find({ server: req.user.server });

            var tablebot = "";
            getBotnapvang.forEach(element => {
                tablebot = "<tr><td hidden=''>" + element._id + "</td><td>" + element.server + "</td>" +
                    "<td>" + element.tnv + "</td>" +
                    "<td>" + (element.map) + "</td>" +
                    "<td>" + (element.khu) + "</td>" +
                    "<td>" + numberWithCommas(element.sothoi) + "</td>" +
                    "</tr>" +
                    tablebot;
            });
            if (tablebot == "") {
            ...
            if (checkNapvang > 0) {
                ...
                if (vang == "" || name == '') {
                    ...
                    if (nameee.length != name.length) {
                        ...
                        try {
                            const rutvangme = await Napvang.find({ tnv: name, status: 0 })
                            if (rutvangme) {
                                await Napvang.updateMany({ tnv: name, status: 0 }, { status: 2 })
                            }
                        } catch { }
            ...
            if (checkNapthoi > 0) {
                ...
                if (vang == "" || name == '') {
                    ...
                    if (nameee.length != name.length) {
                        ...
                        const addnapthoi = new Napthoi({ uid: req.user._id, server: req.user.server, sothoi: sothoi, tnv: name, taikhoan: req.user.name })
                        try {
                            const newnapvang = await addnapthoi.save();
                            var table = "<tr><td hidden=''>" + newnapvang._id + "</td>" + '<td>' + newnapvang.server + '</td>' + '<td>' + newnapvang.tnv + '</td>' + '<td>' + numberWithCommas(newnapvang.sothoi) + '</td>' + '<td>' + getStatus(newnapvang.status) + '</td>' + '<td>' + new Date(Date.parse(newnapvang.time)).toLocaleString() + '</td>' + '<td><i class="fas fa-trash-alt" onclick="return remove(this);"></i></td>' + '</tr>';
                            return res.json({ error: 0, message: "<strong>Thành công</strong> Bạn vui lòng tới địa điểm giao hàng gặp BOT để giao dịch", table: table });
                        } catch {
                            return res.json({ error: 1, message: "<strong>Thất bại: </strong> Lỗi" });
                        }
